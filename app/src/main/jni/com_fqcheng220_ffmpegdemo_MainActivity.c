...
#define LOGI(format,...) __android_log_print(ANDROID_LOG_INFO,"android",format,##__VA_ARGS__)
#endif

//#ifdef ANDROID
//#include <jni.h>
//#include <android/log.h>
//#define LOGE(format, ...)  __android_log_print(ANDROID_LOG_ERROR, "android(>_<)", format, ##__VA_ARGS__)
//#define LOGI(format, ...)  __android_log_print(ANDROID_LOG_INFO,  "android(^_^)", format, ##__VA_ARGS__)
//#else
  	...
  	//Input
    	...
    	for (i = 0; i < ifmt_ctx->nb_streams; i++) {
    		...
    		if (ofmt_ctx->oformat->flags & AVFMT_GLOBALHEADER)
    			out_stream->codec->flags |= AV_CODEC_FLAG_GLOBAL_HEADER;//修改部分
    	}
